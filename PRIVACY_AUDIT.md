...
This document describes the offline architecture of Open Takeout Hub and how to
...
Open Takeout Hub handles the archives a person downloads precisely because they
...
```bash
lsof -a -p "$(pgrep -f 'Open Takeout Hub.app/Contents/MacOS')" -i -P -n   # no rows
```
