...
export function ProblemReport({ info, errors, onError }: ProblemReportProps) {
  ...
  const subject = `Open Takeout Hub ${info?.version ?? ""} on ${environment}${
  ...
  const saveToFile = async () => {
    try {
      const target = await save({
        ...
        defaultPath: "open-takeout-hub-segnalazione.md",
    ...
    <div className="space-y-3 text-sm text-zinc-700 dark:text-zinc-300">
      ...
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        ...
        <span className="selectable font-mono break-all">
          {info?.repository ?? "github.com/skapacraft/open-takeout-hub"}
        </span>
